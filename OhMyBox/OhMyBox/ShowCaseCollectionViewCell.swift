...
class ShowCaseCollectionViewCell: UICollectionViewCell {
    ...
    func initializeTagView(){
        ...
        if tagType! == 0 {
            ...
            self.tagView5?.center = CGPoint(x: self.frame.width/2.50, y: self.frame.height/1.10)
            self.tagView5?.nameTagLabel.text = "VERÃO DE SAIAS"
   // = CGPoint(x: self.frame.width/2.65, y: self.frame.height/1.20)
            self.addSubview(self.tagView5!)
        }
